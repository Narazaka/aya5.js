 ...
 * -----------------------------------------------------------------------
 */
CSelecter::CSelecter(CDuplEvInfo *dc, int aid)
{
	duplctl = dc;
	aindex  = aid;
...
CValue	CSelecter::Output(void)
{
	...
	case CHOICETYPE_NONOVERLAP:
		return duplctl->Choice(areanum, values, CHOICETYPE_NONOVERLAP);
	case CHOICETYPE_SEQUENTIAL:
		return duplctl->Choice(areanum, values, CHOICETYPE_SEQUENTIAL);
...
{
    int choice = (int)(genrand_real2()*(double)values[index].array.size());

    sysfunction.SetLso(choice);
...
CValue CSelecter::StructArray1(int index)
{
    ...
    for(int i = 0; i < values[index].array.size(); i++) {
